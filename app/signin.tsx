...
export default function SignInScreen() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: Platform.OS === 'android' ? 'fortunai://auth/callback' : 'fortunai://profile',
      ...
      if (Platform.OS === 'android') {
        authUrl = authUrl.replace(/https?:\/\/joinfortunai\.com/g, 'fortunai:/');
      ...
      const result = await WebBrowser.openAuthSessionAsync(
        authUrl, 
        Platform.OS === 'android' ? 'fortunai://auth/callback' : 'fortunai://profile'
      );
  ...
  const handleGoogleSignInDifferentAccount = async () => {
    ...
    try {
      ...
      const result = await WebBrowser.openAuthSessionAsync(data.url, 'fortunai://profile');
      console.log('WebBrowser.openAuthSessionAsync result:', result);
  ...
  /* Temporarily disable Apple Sign-In (function commented out)
