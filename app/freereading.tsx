...
function normalizeBirthDate(raw: string): Date {
...
function toPlainTextFromDeepSeek(content: string): string {
  try {
    const obj = JSON.parse(content);
    if (Array.isArray(obj?.fortunes)) {
      return obj.fortunes.map((f: any) => String(f.reading ?? '')).join('\n\n').trim();
    }
  } catch {}
  return content;
}
...
const preprocessMarkdown = (content: string): string => {
  ...
  return content
    // Remove generic disclaimers/openers
    ...
    // Normalize common headings from bold labels to markdown headings
    ...
    // Keep headings and bold; just collapse excessive spacing
    .replace(/\n{3,}/g, '\n\n')
    .trim();
};
...
const GenerateAnalysisButton: React.FC<{
  ...
  const upgradeText = type === 'swot' ? 'SWOT Analysis' : 'Special Analysis';
...
export default function FreeAnalysisPage() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (existingResults) {
        ...
        // Generate initial free reading - pass the authUser.id
        await generateInitialReading(profileData, authUser.id);
      }
    } catch (err: any) {
  ...
  const generateInitialReading = async (userProfile: UserProfile, userId: string) => {
    try {
      const birthDate = normalizeBirthDate(userProfile.birth);
      const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
      const calculator = new BaziCalculator(birthDate, userProfile.gender, timezone, true);
      const analysis = calculator.getCompleteAnalysis();
  
      const prompt = `Provide a comprehensive personality analysis for this individual. Focus on core traits, natural tendencies, and character strengths. Use "you" language and flowing paragraphs. Use natural markdown for headings, bold, and lists. No questions or disclaimers and no Chinese characters.\n\nAnalysis: ${JSON.stringify(analysis)}`;
  
      const raw = await callDeepSeek(prompt);
      const plain = toPlainTextFromDeepSeek(raw);
      const formattedResponse = plain.replace(/\n\s*\n/g, '\n\n').trim();
  ...
  const generateAnalysis = async (type: 'swot' | 'special') => {
    ...
    try {
      ...
      const birthDate = normalizeBirthDate(profile.birth);
      const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
      const calculator = new BaziCalculator(birthDate, profile.gender, timezone, true);
      const analysis = calculator.getCompleteAnalysis();

      let prompt = '';
      if (type === 'swot') {
        prompt = `Provide a detailed SWOT analysis (Strengths, Weaknesses, Opportunities, Threats) for this individual. Use "you" language and flowing paragraphs. Use natural markdown for headings, bold, and lists. No questions or disclaimers and no Chinese characters.\n\nAnalysis: ${JSON.stringify(analysis)}`;
      } else {
        prompt = `Provide special life guidance and insights for this individual. Focus on unique opportunities, hidden potentials, and personalized advice. Use "you" language and flowing paragraphs. Use natural markdown for headings, bold, and lists. No questions or disclaimers and no Chinese characters.\n\nAnalysis: ${JSON.stringify(analysis)}`;
      }
